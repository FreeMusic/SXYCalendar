...
@interface SXYDateToolsObject ()

@property (nonatomic, strong) NSDateFormatter *formate;

@end

static SXYDateToolsObject *yxDate = nil;

@implementation SXYDateToolsObject

+ (SXYDateToolsObject *)manager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        yxDate = [[SXYDateToolsObject alloc] init];
    });
    return yxDate;
}
...
- (NSDate *)getLastdayOfTheWeek:(NSDate *)date {
    ...
    return [self getEarlyOrLaterDate:date LeadTime:7 - week Type:TimeType_Month];
...
- (NSDate *)getEarlyOrLaterDate:(NSDate *)currentDate LeadTime:(NSInteger)lead Type:(TimeType )timeType {
    ...
    if (timeType == TimeType_Year) {
        [dateCom setYear:lead];
    } else if (timeType == TimeType_Month) {
        [dateCom setMonth:lead];
    } else if (timeType == TimeType_Day) {
        [dateCom setDay:lead];
    } else if (timeType == TimeType_Hour) {
        [dateCom setHour:lead];
    } else if (timeType == TimeType_Minute) {
        [dateCom setMinute:lead];
    } else if (timeType == TimeType_Second) {
